  ...
  return (
    <div className="bg-vault-accent-dim border border-vault-accent-border rounded-[8px] p-5 flex flex-col gap-3 my-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <textarea
        ...
        className="w-full bg-transparent border-none focus:outline-none text-[18px] text-vault-text resize-none min-h-[80px] placeholder:text-vault-text-3"
      />
      <div className="flex justify-end">
        <button
          onClick={saveToJournal}
          disabled={loading || !reaction.trim()}
          className="bg-vault-accent text-[#0D0D0F] text-[10px] uppercase tracking-normal py-1.5 px-4 rounded-[4px] hover:bg-vault-accent-2 disabled:opacity-50 transition-colors"
        >
