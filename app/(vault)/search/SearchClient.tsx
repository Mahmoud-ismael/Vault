...
export default function SearchClient() {
  ...
  return (
    <div className="w-full min-h-screen bg-vault-bg py-20 px-8">
      <div className="max-w-[780px] mx-auto flex flex-col gap-8">
        ...
        <div className={`flex flex-col gap-4 transition-all duration-500 ${activeQuery ? '' : 'transform translate-y-4'}`}>
          <form onSubmit={handleSearch} className="relative group">
            <SearchIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-vault-text-3 transition-colors group-focus-within:text-vault-accent" />
            <input 
              value={query}
            ...
            <button 
              type="submit"
              disabled={!query.trim()}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-vault-accent text-[#0D0D0F] flex items-center justify-center hover:bg-vault-accent-2 disabled:opacity-50 transition-colors"
            >
              <ArrowRight className="w-4 h-4" />
            </button>
          </form>

          <div className="flex border border-vault-border rounded-[6px] overflow-hidden w-fit mx-auto shadow-sm">
        ...
        {activeQuery && (mode !== 'Vault') && (aiResponse || loading) && (
          <div className="flex flex-col gap-8 mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* AI Response Card */}
            <div className="bg-vault-bg-2 border-2 border-vault-accent-dim rounded-[12px] p-8 shadow-sm relative overflow-hidden">
